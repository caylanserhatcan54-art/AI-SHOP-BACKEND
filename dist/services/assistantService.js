...
export async function generateSmartReply(shopId, msg) {
    try {
        ...
        const foundProduct = products.find(p => minimizedMsg.includes(p.title?.toLowerCase().split(" ")[0]) ||
...
function createProductExplanation(product) {
    let response = `💡 Bu ürün hakkında bilgi vereyim:\n\n`;
    response += `✨ **${product.title}**\n`;
    if (product.price)
        response += `💰 Güncel Fiyatı: ${product.price}\n`;
    if (product.image)
        response += `🖼️ Görsel Linki: ${product.image}\n`;
...
function generateCombinationAdvice(products) {
...
function generatePriceInfo(products) {
