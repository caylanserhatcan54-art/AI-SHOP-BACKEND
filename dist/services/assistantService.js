...
function findMatchingProducts(msg, products) {
    ...
    if (normMsg.includes("mont") || normMsg.includes("kaban")) {
        return products.filter(p => normalizeText(p.title || "").includes("mont") ||
            normalizeText(p.title || "").includes("kaban"));
    }
    // SADECE AYAKKABI
    if (normMsg.includes("ayakkabi") || normMsg.includes("ayakkabı")) {
        return products.filter(p => normalizeText(p.category || "") === "ayakkabi");
    }
...
function boughtTogether(main, products) {
    if (!main)
        return "";
    const related = products.filter(p => p.id !== main.id && p.category === main.category)[0];
    if (!related)
        return "";
    return `
...
function crossSell(main, products) {
    if (!main)
        return "";
    if (main.category === "ayakkabi") {
        const pantolon = products.find(p => /pantolon|jean/.test(normalizeText(p.title || "")));
        if (pantolon) {
...
function giftBrain(msg) {
    const t = normalizeText(msg);
    if (!t.includes("hediye"))
        return "";
    if (t.includes("anne"))
        return "🎁 Anneler için zarif ve günlük kullanılabilir ürünler daha çok beğenilir.";
    if (t.includes("baba"))
        return "🎁 Babalar için sade ve kullanışlı ürünler daha mantıklıdır.";
    if (t.includes("sevgili"))
        return "❤️ Sevgili için biraz daha özel ve tarz yansıtan ürünler tercih edilir.";
    return "🎁 Hediye alırken kullanışlılık + tarz uyumu en güvenli yaklaşımdır.";
}
/* ---------- 7️⃣ AI ÇIKTI BİRLEŞTİRİCİ ---------- */
function enrichAnswer(base, main, products, msg) {
    let out = base;
    out += "\n\n" + salesPitch();
...
function generateGeneralInfoAnswer(msg) {
    const t = normalizeText(msg);
...
function detectWorldCategory(msg) {
    const t = normalizeText(msg);
    if (/bahçe|çim|budama|kesme|tırpan|çalı/.test(t))
        return "bahce";
    if (/temizlik|krem|deterjan|çamaşır|bulaşık/.test(t))
        return "temizlik";
    if (/hırdavat|matkap|vida|pense|anahtar/.test(t))
        return "hirdavat";
    if (/şampuan|krem|traş|bakım|cilt/.test(t))
        return "kozmetik";
    if (/ilaç|ağrı|vitamin|takviye/.test(t))
        return "saglik";
    if (/tencere|tava|mutfak|ocak/.test(t))
        return "mutfak";
    if (/masa|sandalye|koltuk|baza|komodin/.test(t))
        return "mobilya";
    if (/telefon|laptop|kulaklık|tablet/.test(t))
        return "elektronik";
    if (/spor|fitness|koşu|yoga/.test(t))
        return "spor";
    if (/perde|halı|dekor|avize/.test(t))
        return "ev-dekorasyon";
    return "genel";
}
/* ---------- MAĞAZA KATEGORİSİ VAR MI? ---------- */
function storeHasCategory(storeProducts, worldCategory) {
...
function worldKnowledgeAnswer(msg) {
