...
function detectCategory(text) {
    ...
    if (t.includes("tshirt") || t.includes("tişört") || t.includes("t-shirt"))
        return "tshirt";
    if (t.includes("kazak") || t.includes("triko") || t.includes("hirka") || t.includes("hırka"))
        return "kazak";
    if (t.includes("mont") || t.includes("kaban") || t.includes("parka") || t.includes("ceket"))
        return "mont";
    if (t.includes("pantolon") || t.includes("jean") || t.includes("kot"))
        return "pantolon";
    if (t.includes("ayakkabi") || t.includes("ayakkabı") || t.includes("sneaker") || t.includes("bot"))
        return "ayakkabi";
    if (t.includes("bardak") || t.includes("kupa") || t.includes("mug") || t.includes("fincan") || t.includes("termos") || t.includes("matara"))
        return "bardak";
    if (t.includes("tencere") || t.includes("tava") || t.includes("granit") || t.includes("döküm") || t.includes("dokum") || t.includes("celik") || t.includes("çelik"))
        return "tencere";
    if (t.includes("kedi maması") || t.includes("kedi mamasi"))
        return "kedi-mamasi";
    if (t.includes("köpek maması") || t.includes("kopek mamasi") || t.includes("köpek mamasi"))
        return "kopek-mamasi";
    return "diger";
}
function extractBasicAttributes(text) {
    const t = normalizeTR(text);
    const attributes = {};
    // gender
    if (t.includes("erkek"))
        attributes.gender = "erkek";
    else if (t.includes("kadin") || t.includes("kadın"))
        attributes.gender = "kadin";
    else if (t.includes("unisex"))
        attributes.gender = "unisex";
    else if (t.includes("cocuk") || t.includes("çocuk"))
        attributes.gender = "cocuk";
    ...
    if (brandMatch?.[1])
        attributes.brand = cleanSpaces(brandMatch[1]);
    // color
    ...
    if (sizeOps.length)
        attributes.sizeOptions = sizeOps;
    // material
    if (t.includes("organik pamuk"))
        attributes.material = "organik pamuk";
    else if (t.includes("pamuk"))
        attributes.material = "pamuk";
    else if (t.includes("polyester"))
        attributes.material = "polyester";
    else if (t.includes("deri"))
        attributes.material = "deri";
    else if (t.includes("cam"))
        attributes.material = "cam";
    else if (t.includes("plastik"))
        attributes.material = "plastik";
    else if (t.includes("celik") || t.includes("çelik"))
        attributes.material = "çelik";
    // capacity
    const cap = t.match(/(\d{2,5})\s*(ml|l|lt|litre)\b/);
    if (cap)
        attributes.capacity = `${cap[1]} ${cap[2]}`;
    return attributes;
}
