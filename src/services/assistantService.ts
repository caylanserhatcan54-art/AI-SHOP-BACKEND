...
const DAILY_TALK_PATTERNS: { regex: RegExp; answer: string }[] = [
  {
    ...
    answer:
      "İyiyim, seninle ilgileniyorum 😊 Sen nasılsın? Bugün ne bakıyoruz, ürün mü, kombin mi?",
  },
  {
...
function findMatchingProducts(msg: string, products: Product[]): Product[] {
  ...
  const tokens = normMsg
    ...
    .filter(
      (t) =>
        t.length > 2 && !TURKISH_STOP_WORDS.includes(t) && t.trim().length > 0
    );
...
function buildReplyForIntent(
  ...
  if (
    /hangisi mantikli|mantikli hangisi|hangisini alayim|hangisini secmeliyim/i.test(
      userMessage
    )
  ) {
