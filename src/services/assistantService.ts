...
function formatName(n: string | null) {
  if (!n) return "";
  return `${n} `;
}
 ...
 * ---------------------------------------------- */
function rejectAbsurd(msg: string): string | null {
  ...
  for (const r of rules) {
    if (r.keys.every(k => t.includes(normalizeText(k)))) return r.msg;
  }
  return null;
}
 ...
 * SATIN ALMA NİYETİ
 * ---------------------------------------------- */
function detectPurchaseIntent(t: string): "HIGH"|"MID"|"LOW" {
  t = normalizeText(t);
 ...
 * DUYGU ANALİZİ
 * ---------------------------------------------- */
function detectSentiment(t: string): "NEG"|"POS"|"NEU" {
  t = normalizeText(t);
...
function sentimentTone(s: "NEG"|"POS"|"NEU") {
 ...
 * ---------------------------------------------- */
function calmDown(t: string): string | null {
  t = normalizeText(t);
...
function findMatchingProducts(msg: string, products: Product[]): Product[] {
  ...
  if (!tokens.length) return products.slice(0, 5);

  let scored = products.map(p => {
    const title = normalizeText(p.title || "");
    let score = 0;
    for (const tok of tokens) if (title.includes(tok)) score += 2;
    return { p, score };
  });
...
function formatProduct(p: Product): string {
  const img = (p as any).image || (p as any).imageUrl || "";
  return `✨ **${p.title}**
 ...
 * KOMBİN ÖNERİ
 * ---------------------------------------------- */
function buildCombination(main: Product, all: Product[]) {
  const sec = all.find(x => x.id !== main.id);
  if (!sec) return formatProduct(main);
 ...
 * ANA YANIT ÜRETİCİ
 * ---------------------------------------------- */
function buildReply(intent: Intent, msg: string, products: Product[], name: string | null) {
  ...
  if (intent === "SMALL_TALK") {
    for (const p of SMALL_TALK) {
      if (p.regex.test(msg)) return displayName + p.answer;
    }
  ...
  if (/hangisi mantikli|hangisini alayim/.test(msg)) {
    ...
    if (two.length < 2) return formatProduct(main!);
    return (
...
if (intent === "ASK_RECOMMENDATION") {
  ...
  return (
    "Sana uygun birkaç ürün öneriyorum 🌟\n\n" +
    list.map((p,i)=>`#${i+1}\n${formatProduct(p)}`).join("\n\n")
  );
}
 ...
 * ---------------------------------------------- */
function buildFull(intent: Intent, msg: string, products: Product[], name: string|null) {
 ...
 * ---------------------------------------------- */
export async function generateSmartReply(shopId: string, userMessage: string) {
...
export async function getAssistantReply(shopId: string, msg: string) {
  return generateSmartReply(shopId, msg);
}

export async function getAIResponse(shopId: string, msg: string) {
  return generateSmartReply(shopId, msg);
}
