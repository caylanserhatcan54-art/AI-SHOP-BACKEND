...
const MATCHING_PATTERNS =
  /(ustune ne gider|üstüne ne gider|altina ne gider|altına ne gider|bununla ne gider|buna ne uyar|uyumlu ne var|yanina ne olur|yanına ne olur|yakisir mi|yakışır mı)/i;
...
const GUIDANCE_PATTERNS =
  /(kararsız|ne alacağımı bilmiyorum|emin değilim|önerir misin|ne önerirsin|fikir ver|yardımcı olur musun|hangisi daha güzel|hangisi iyi|hangisini alayım|bunu mu alsam|sence hangisi|bu mu daha iyi|sen olsan hangisini alırdın|kararsız kaldım|seçemedim|aklım karıştı)/i;

if (GUIDANCE_PATTERNS.test(msg)) {
  return {
    reply:
      "Sorun değil 🙂 birlikte netleştirelim.\n\n" +
  ...
  /(buna değer mi|ofiste kullanılır mı|ev için uygun mu|iş görür mü|alınır mı|mantıklı mı|is it worth|worth buying|should i buy|is it good)/i;
  ...
  /(iphone|android|uyumlu mu|uyar mı|olur mu|compatible|does it fit|works with|supported)/i;
  ...
  if (group === "ayakkabi") targetGroups = ["ust", "alt", "aksesuar"];
  else if (group === "ust") targetGroups = ["alt", "ayakkabi"];
  else if (group === "alt") targetGroups = ["ust", "ayakkabi"];
  ...
  return {
    reply:
      "Buna uyumlu olabilecek birkaç parça çıkardım 👇\n" +
